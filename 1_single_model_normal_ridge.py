...
if __name__=="__main__":
    ...
    normal_ridge = make_pipeline(RobustScaler(), SVR())
    normal_ridge_dic = {
    ...
    for rd in range(3):
        ...
        print("30 generations are required to find the best individual. Please wait~~")
        selector.generate(60,ngen=30,cxpb=0.1,mutxpb=0.8)
        ...
        for i in range(len(record)):
            ...
            if final_score < record_score:
                ...
                with open("normal_ridge/round_"+str(rd)+"_item_"+str(count)+"_cv_score_"+str(record_score)+".pkl","wb") as f:
                ...
                submission.to_csv("normal_ridge/round_"+str(rd)+"_item_"+str(count)+"_cv_score_"+str(record_score)+".csv")
                count+=1
                print("="*60)
                print("cv_score is:")
                print(record_score)
        ...
        bench_model = make_pipeline(RobustScaler(), Ridge(**best_para))
        ...
        with open("normal_ridge/round_"+str(rd)+"_item_"+str(count)+"_cv_score_"+str(tunned_result["score"])+".pkl","wb") as f:
        ...
        submission = extract_pd(test_x,prediction)  
        submission.to_csv("normal_ridge/round_"+str(rd)+"_item_"+str(count)+"_cv_score_"+str(tunned_result["score"])+".csv")
